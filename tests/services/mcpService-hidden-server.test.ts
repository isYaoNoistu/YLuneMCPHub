...
const mockUserContextService = {
  ...
  getEffectiveAccess: jest.fn<() => any>(() => cachedAccess),
  setEffectiveAccess: jest.fn((access: any) => {
    cachedAccess = access;
  }),
...
const adminUser = { username: 'admin', isAdmin: true, password: '' };
const alicePublicGroup = {
  id: 'g-public',
  name: 'public-only',
  owner: 'admin',
  members: ['alice'],
  servers: [{ name: 'PublicServer', tools: 'all', prompts: 'all', resources: 'all' }],
};
...
describe('hidden-server tool/prompt errors (issue #1103)', () => {
  ...
  const setFixture = () => {
    ...
    mockGroupDao.findByMember.mockResolvedValue([alicePublicGroup]);
    mockGroupDao.findByName.mockResolvedValue(undefined);
    mockGroupDao.findById.mockResolvedValue(undefined);
  };
  ...
  it('still routes a member-granted public server tool to invocation (not "not available")', async () => {
    // Alice is a member of a group that includes PublicServer. With no live
    // client the invocation fails operationally ("Client not found"), NOT with
    // the unified unavailable error — a guard against over-broad unification
  ...
  it('lets an admin invoke any enabled server without being a group member', async () => {
