...
const requireAdmin = (res: Response): boolean => {
  if (UserContextService.getInstance().isAdmin()) {
    return true;
  }
  res.status(403).json({
    success: false,
    message: 'Only administrators can manage groups',
  });
  return false;
};

const isValidMembers = (members: unknown): members is string[] | undefined => {
  return members === undefined || (Array.isArray(members) && members.every((item) => typeof item === 'string'));
};
...
export const createNewGroup = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { name, description, servers, members } = req.body;
    if (!name) {
      res.status(400).json({
        success: false,
    ...
    if (!isValidMembers(members)) {
      res.status(400).json({
        success: false,
        message: 'Members must be an array of usernames',
      });
      return;
    }
    ...
    for (const groupData of groups) {
      ...
      try {
        ...
        const newGroup = await createGroup(
          ...
          serverList,
          defaultOwner,
          normalizeMembers(groupData.members),
        );
...
export const updateExistingGroup = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (members !== undefined) {
      ...
      updateData.members = normalizeMembers(members);
    }
    if (servers !== undefined) {
    ...
    if (Object.keys(updateData).length === 0) {
      res.status(400).json({
        success: false,
        message: 'At least one field (name, description, servers, or members) is required to update',
      });
      return;
    }
