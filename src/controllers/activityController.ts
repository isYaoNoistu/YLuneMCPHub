...
export const getActivityUsage = async (req: Request, res: Response): Promise<void> => {
  if (!(await requireAdmin(req, res))) return;
  ...
  try {
    ...
    const data = await activityDao.getUsage(days, limit);
    ...
    logger.error('Error fetching activity usage:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch activity usage',
    });
  }
};
