...
export const getFilteredServerInfosForGroup = async (
  ...
  const access = await ensureEffectiveAccess();
  ...
  if (group) {
    ...
    if (foundGroup) {
      if (
        !access.unrestricted &&
        (!currentUser?.username || !isGroupMember(foundGroup, currentUser.username))
      ) {
    ...
    serverConfigs = [...access.serversByName.values()];
  }
  ...
  for (const serverInfo of runtimeServers) {
    ...
    if (!group) {
      if (access.unrestricted || isServerGranted(access, serverInfo.name)) {
    ...
    if (serverNamesInGroup.size === 0) {
      if (
        ...
        (access.unrestricted || isServerGranted(access, serverInfo.name))
      ) {
        filteredServerInfos.push(serverInfo);
      }
      continue;
...
const getGroupServerConfig = async (
  ...
  if (!group) {
    const access = UserContextService.getInstance().getEffectiveAccess();
    if (access && !access.unrestricted) {
      return access.serversByName.get(serverName);
    }
    return undefined;
  }

  return getServerConfigInGroup(group, serverName);
};
