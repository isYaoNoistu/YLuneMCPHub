...
export const normalizeMembers = (members?: string[]): string[] => {
...
const canManageGroups = (): boolean => {
  return UserContextService.getInstance().isAdmin();
};

const canMutateGroup = (_group: IGroup): boolean => {
...
export const getAllGroups = async (): Promise<IGroup[]> => {
  ...
  return groupDao.findByMember(currentUser.username);
};
