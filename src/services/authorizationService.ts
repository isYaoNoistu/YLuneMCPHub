...
import { getEffectiveAccess, isServerGranted } from './groupAccessService.js';
...
// (config.read/manage). Discover/invoke follow group membership, not server
// visibility. Sync `can()` uses the request-scoped cache; `canInvoke()` loads it.
...
export class AuthorizationService {
  can(
    ...
    switch (action) {
      ...
      case 'server.invoke': {
        ...
        const cached = UserContextService.getInstance().getEffectiveAccess();
        if (!cached || !server.name) {
          return false;
        }
        return isServerGranted(cached, server.name);
      }
    }
  }
  ...
  async canInvoke(
    ...
    return isServerGranted(access, server.name);
  }
}
