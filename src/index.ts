...
async function boot() {
  try {
    ...
    const allowFileSettings =
      process.env.NODE_ENV === 'development' ||
      process.env.NODE_ENV === 'test' ||
      Boolean(process.env.JEST_WORKER_ID);
    if (!useDatabase && !allowFileSettings) {
      logger.error(
        'YLune stores servers, users, groups, and keys in PostgreSQL. Set DB_URL (or USE_DB=true). mcp_settings.json is not the operational store.',
      );
      process.exit(1);
    }
    if (!useDatabase && allowFileSettings) {
      logger.warn(
        'File settings mode is for local/test only. Operational config belongs in PostgreSQL (DB_URL).',
      );
    }
    if (useDatabase) {
    ...
    if (getCachedSystemConfig()?.routing?.skipAuth) {
      logger.warn(
        '⚠️  SECURITY WARNING: routing.skipAuth is ENABLED — dashboard authentication is DISABLED and ALL API callers are treated as admins.\n' +
          ...
          '⚠️  Never expose this instance to a network. To disable: turn off skipAuth in system config (database).',
      );
    }
