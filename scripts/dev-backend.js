...
function emptySettingsJson() {
  return `${JSON.stringify({ mcpServers: {}, users: [], groups: [] }, null, 2)}\n`;
}
...
function prepareDevSettingsFile(settingsPath) {
  ...
  fs.writeFileSync(settingsPath, emptySettingsJson(), 'utf8');
  return true;
}
...
function startBackend(extraArgs) {
  ...
  console.log('[dev] File settings are local/test only; set DB_URL to store config in PostgreSQL');

  // Windows + Node 20+：直接 spawn *.cmd 会 EINVAL，必须走 shell
