  ...
  return (
    <section className="settings-section" data-section={id} aria-labelledby={`settings-${id}`}>
      <div className="settings-section-head">
        <h2 className="settings-section-title" id={`settings-${id}`}>
          {title}
        </h2>
        {extra}
      </div>
...
const SettingsPage: React.FC = () => {
  ...
  const settingsNav: { id: Parameters<typeof toggleSection>[0]; label: string }[] = [
    { id: 'bearerKeys', label: t('settings.bearerKeysSectionTitle') || 'Keys' },
    { id: 'routingConfig', label: t('pages.settings.routeConfig') },
    { id: 'smartRoutingConfig', label: t('pages.settings.smartRouting') },
    { id: 'toolResultCompressionConfig', label: t('settings.toolResultCompressionTitle') },
    { id: 'oauthServerConfig', label: t('pages.settings.oauthServer') },
    { id: 'betterAuthConfig', label: t('settings.betterAuthTitle') || 'Better Auth' },
    { id: 'installConfig', label: t('pages.settings.installConfig') },
    { id: 'nameSeparator', label: t('settings.nameSeparatorLabel') },
    { id: 'password', label: t('pages.settings.password') },
    { id: 'exportConfig', label: t('settings.exportMcpSettings') },
  ];
    ...
    <div className="hub-page-stack settings-page">
      <div className="hub-page-head">
        <div>
          ...
          <p className="hub-sub">{t('settings.subtitle') || '网关、鉴权与运行参数。每次只打开一块，避免挤在一页。'}</p>
        </div>
      </div>

      <div className="settings-layout">
      <nav className="settings-nav" aria-label={t('pages.settings.title')}>
        {settingsNav.map((item) => (
          <button
            key={item.id}
            type="button"
            className={`side-link${sectionsVisible[item.id] ? ' is-active' : ''}`}
      ...
      {/* Bearer Keys Settings */}
        <SettingsSection
          id="bearerKeys"
          visible={sectionsVisible.bearerKeys}
          title={t('settings.bearerKeysSectionTitle') || 'Bearer authentication keys'}
        >
              {createdBearerToken && (
                <div className="settings-banner">
                  ...
                  <code className="settings-token">{createdBearerToken}</code>
                </div>
              )}
              {isAdmin && <>
              ...
              {isAdmin && <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {bearerKeys.length > 0 && bearerKeyScopeFilterOptions.length > 1 && (
                    <div className="flex items-center gap-2">
                      <label htmlFor="bearer-key-scope-filter" className="ylune-label" style={{ margin: 0 }}>
              ...
              {bearerKeys.length === 0 ? (
                ...
                <div className="settings-table-wrap">
                  <table className="settings-table">
                    <thead>
                      <tr>
                        <th scope="col">{t('settings.bearerKeyName') || 'Name'}</th>
                        <th scope="col">{t('settings.bearerKeyToken') || 'Token'}</th>
                        <th scope="col">{t('settings.bearerKeyEnabled') || 'Status'}</th>
                        <th scope="col">{t('settings.bearerKeyAccessType') || 'Access Scope'}</th>
              ...
              {showAddBearerKeyForm && (
                <div className="settings-form-card">
                    <h3 className="settings-label" style={{ marginBottom: 16 }}>
                      {t('settings.addBearerKey') || 'Add bearer key'}
                    </h3>
                    ...
                    <div className="flex flex-wrap items-end gap-4 mb-2">
                      <div className="w-40">
                        ...
                        <div className="settings-row" style={{ padding: 0, border: 0, gap: 10 }}>
                          <span
                            className={`hub-status ${newBearerKey.enabled ? 'ok' : 'muted'}`}
                          >
                      ...
                      {isAdmin && newBearerKey.kind === 'system' && newBearerKey.accessType !== 'custom' && (
                        <div className="flex-1 min-w-[200px]">
                          <label
                            className={`ylune-label${newBearerKey.accessType === 'all' ? ' is-muted' : ''}`}
                          >
      ...
      <PermissionChecker permissions={PERMISSIONS.SETTINGS_SMART_ROUTING}>
        <SettingsSection
          id="smartRoutingConfig"
          visible={sectionsVisible.smartRoutingConfig}
          title={t('pages.settings.smartRouting')}
          extra={
            <span className={`hub-status ${smartRoutingConfig.enabled ? 'ok' : 'muted'}`}>
              <span className="hub-dot" />
        ...
        >
                ...
                <div className="settings-hero">
                  <div className="settings-hero-top">
                    <div className="settings-hero-status">
                      <div className="relative" style={{ width: 44, height: 44, flex: 'none' }}>
                        <div
                          className="absolute inset-0 rounded-lg grid place-items-center"
                          style={{
                            background: smartRoutingConfig.enabled
                              ? 'color-mix(in oklch, var(--hub-ok) 16%, var(--hub-bg-2))'
                              : 'var(--hub-bg-2)',
                    ...
                    <label className="settings-hero-toggle">
                      <span>{t('settings.enableSmartRouting')}</span>
                      <Switch
                        disabled={loading}
                  ...
                  <div className="settings-hero-endpoint">
              ...
              <div className="hub-card">
                <h3 className="hub-card-title" style={{ marginBottom: 6 }}>
                  {t('settings.smartRoutingWorkflow') || 'Workflow'}
                </h3>
                <p className="hub-sub" style={{ marginTop: 0, marginBottom: 20 }}>
                ...
                <div className="settings-flow grid items-center">
                  {[
      ...
      <PermissionChecker permissions={PERMISSIONS.SETTINGS_SMART_ROUTING}>
        <SettingsSection
          id="toolResultCompressionConfig"
          visible={sectionsVisible.toolResultCompressionConfig}
          title={t('settings.toolResultCompressionTitle') || 'Tool Result Compression'}
          extra={
            <span className={`hub-status ${toolResultCompressionConfig.enabled ? 'ok' : 'muted'}`}>
              <span className="hub-dot" />
      ...
      <PermissionChecker permissions={PERMISSIONS.SETTINGS_OAUTH_SERVER}>
        <SettingsSection
          id="oauthServerConfig"
          visible={sectionsVisible.oauthServerConfig}
          title={t('pages.settings.oauthServer')}
        >
              <div className="settings-row">
                <div>
                  <h3 className="settings-label">{t('settings.enableOauthServer')}</h3>
                  <p className="settings-help">
              ...
              <div className="settings-row">
                <div>
                  <h3 className="settings-label">{t('settings.requireClientSecret')}</h3>
                  <p className="settings-help">
              ...
              <div className="settings-row">
                <div>
                  <h3 className="settings-label">{t('settings.requireState')}</h3>
                  <p className="settings-help">{t('settings.requireStateDescription')}</p>
                </div>
                <Switch
                  disabled={loading || !oauthServerConfig.enabled}
              ...
              <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">{t('settings.accessTokenLifetime')}</h3>
                  <p className="settings-help">
              ...
              <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">{t('settings.allowedScopes')}</h3>
                  <p className="settings-help">{t('settings.allowedScopesDescription')}</p>
                </div>
                <div className="flex items-center gap-3">
                  <input
                    type="text"
              ...
              <div className="settings-block space-y-4">
      ...
      <PermissionChecker permissions={PERMISSIONS.SETTINGS_SYSTEM_CONFIG}>
        <SettingsSection
          id="betterAuthConfig"
          visible={sectionsVisible.betterAuthConfig}
          title={t('settings.betterAuthTitle') || 'Better Auth'}
        >
              <div
              ...
              <div className="settings-block space-y-4">
                ...
                <div className="settings-row">
                  <div>
                    <h4 className="settings-label">Google</h4>
                    <p className="settings-help">
                ...
                <div className="settings-row">
                  <div>
                    <h4 className="settings-label">GitHub</h4>
                    <p className="settings-help">
                ...
                <div className="settings-row">
                  <div>
                    <h4 className="settings-label">OIDC</h4>
                    <p className="settings-help">
      ...
      <PermissionChecker permissions={PERMISSIONS.SETTINGS_SYSTEM_CONFIG}>
        <SettingsSection
          id="nameSeparator"
          visible={sectionsVisible.nameSeparator}
          title={t('settings.systemSettings')}
        >
              <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">{t('settings.nameSeparatorLabel')}</h3>
                  <p className="settings-help">{t('settings.nameSeparatorDescription')}</p>
                </div>
                <div className="flex items-center gap-3">
                  <input
                    type="text"
      ...
      <PermissionChecker permissions={PERMISSIONS.SETTINGS_ROUTE_CONFIG}>
        <SettingsSection
          id="routingConfig"
          visible={sectionsVisible.routingConfig}
          title={t('pages.settings.routeConfig')}
        >
              <div className="settings-row">
                <div>
                  <h3 className="settings-label">{t('settings.enableGlobalRoute')}</h3>
                  <p className="settings-help">
              ...
              <div className="settings-row">
                <div>
                  <h3 className="settings-label">{t('settings.skipAuth')}</h3>
                  <p className="settings-help">{t('settings.skipAuthDescription')}</p>
                </div>
                <Switch
                  disabled={loading}
              ...
              <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">{t('settings.jsonBodyLimit')}</h3>
                  <p className="settings-help">{t('settings.jsonBodyLimitDescription')}</p>
                </div>
                <div className="flex items-center gap-3">
                  <input
                    type="text"
      ...
      <PermissionChecker permissions={PERMISSIONS.SETTINGS_INSTALL_CONFIG}>
        <SettingsSection
          id="installConfig"
          visible={sectionsVisible.installConfig}
          title={t('settings.installConfig')}
        >
              <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">{t('settings.baseUrl')}</h3>
                  <p className="settings-help">{t('settings.baseUrlDescription')}</p>
                </div>
                <div className="flex items-center gap-3">
                  <input
                    type="text"
              ...
              <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">{t('settings.pythonIndexUrl')}</h3>
                  <p className="settings-help">{t('settings.pythonIndexUrlDescription')}</p>
                </div>
                <div className="flex items-center gap-3">
                  <input
                    type="text"
              ...
              <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">{t('settings.npmRegistry')}</h3>
                  <p className="settings-help">{t('settings.npmRegistryDescription')}</p>
                </div>
                <div className="flex items-center gap-3">
                  <input
                    type="text"
              ...
              <div className="settings-block">
                <div className="mb-2">
                  <h3 className="settings-label">{t('settings.clearCache')}</h3>
                  <p className="settings-help">{t('settings.clearCacheDescription')}</p>
                </div>
                <button
      ...
      {/* Change Password */}
        <SettingsSection
          id="password"
          visible={sectionsVisible.password}
      ...
      <PermissionChecker permissions={PERMISSIONS.SETTINGS_EXPORT_CONFIG}>
        <SettingsSection
          id="exportConfig"
          visible={sectionsVisible.exportConfig}
          title={t('settings.exportMcpSettings')}
        >
              <div className="settings-block">
                <div className="mb-4">
                  <h3 className="settings-label">{t('settings.mcpSettingsJson')}</h3>
                  <p className="settings-help">
