...
![用户权限：按人勾选服务器、工具和凭据](docs/images/add-user.png)

浅色同一张表单：

![用户权限（浅色）](docs/images/add-user-light.png)
...
| 身份            | 默认 `/mcp`              | 改授权      |
| ------------- | ---------------------- | ------------- |
| 管理员（开了 MCP 的 Key） | 全部已启用服务 + 内置 `ylune` | 可以            |
| 普通用户（用户 Key）  | 该用户勾过的工具；空清单则没有工具       | 不可以           |
| 系统 Key `all`  | 全部已启用上游服务，不含 `ylune`     | 不可以           |
