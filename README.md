...
- **凭据中心** — 一条凭据就是一个键值包，默认带出 HOST / PORT / TOKEN，绑定 MCP 可选且不改字段。绑上之后，创建用户或管理员时能选它。同一页可对已绑凭据测试 `listTools`。调用时覆盖该 MCP 的环境变量，不往工具参数里塞密文或租约 ID。主密钥是 `YLUNE_MASTER_KEY`。列表只显示变量名和「已配置」，没有「查看明文」。
